...
- AI 모드의 악기별 볼륨 카드에서 위아래로 스크롤하다 슬라이더를 건드리면 값이 바뀔 수 있습니다(슬라이더가 누른 지점으로 즉시 이동하는 동작). 밸런스가 틀어졌으면 '초기화'로 되돌리세요.
- 같은 유튜브 링크를 다시 추가하면 곡이 중복 등록됩니다. 내려받은 원본은 재사용하지만 목록 항목은 새로 생깁니다.
